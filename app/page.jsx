...
export default function Home() {
  ...
  const getAllPins=async()=>{
    ...
    if (search2 == '') {
      ...
      setListOfPins([])
      const q=query(collection(db, 'pinterest-post'), where('category', '==', search2));
      const querySnapshot = await getDocs(q);
      querySnapshot.forEach((doc) => {
  ...
  return (
    ...
    <div className='bg-[#D2D3D3] p-3 px-2 text-black
         gap-3 items-center rounded-full w-3/6 h-12 hidden md:flex border border-[#9651DB] border-4 absolute top-2 ml-96'>
